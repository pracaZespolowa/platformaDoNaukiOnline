...
function Reservations({ user, setUser }) {
    ...
    const handleAcceptReservation = async (reservationId) => {
        try {
            const response = await fetch(`http://localhost:4000/reservation/accept/${reservationId}`, {
                method: "GET",
                headers: {
                    "Content-Type": "application/json",
                },
    ...
    const handleDeclineReservation = async (reservationId) => {
        try {
            const response = await fetch(`http://localhost:4000/reservation/decline/${reservationId}`, {
                method: "DELETE",
                headers: {
                    "Content-Type": "application/json",
                },
                    ...
                    <ul>
                        ...
                        {reservations.map((reservation, index) => (
                            <li key={index} className="reservation-item">
                                ...
                                <p>Godzina {reservation.date.hour}.{reservation.date.minutes}</p>
